...
EXPORT xbee_err xbee_setup(struct xbee **ret_xbee, char *mode, ...) {
	...
	if ((ret = xbee_modeImport(xbee->mode, &xbee->conTypes)) != XBEE_ENONE) goto die;
	...
	if ((ret = xbee_threadStart(xbee, NULL, 150000, xbee_rxHandler, xbee->fBlock)) != XBEE_ENONE)                     goto die;
