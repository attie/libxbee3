...
$(addprefix $(SYS_MANDIR)/,$(addsuffix .gz,$(SYS_MANLINKS))): $(SYS_MANDIR)/%.gz: $(MANDIR)/%
	ln -sf $(shell readlink $^).gz $@
...
release: all
	tar -acvf $(LIBOUT)_v$(LIBFULLREV)_`date +%Y-%m-%d`_`git rev-parse --verify --short HEAD`_`uname -m`.tar.bz2 --transform='s#^#libxbee/#SH' $(RELEASE_ITEMS)
