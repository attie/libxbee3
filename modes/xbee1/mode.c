...
XBEE_DECLARE_CONTYPE(xbee1, transmitStatus) = {
...
xbee_err xbee_s1_modemStatus_rx_func(struct xbee *xbee, void *arg, unsigned char identifier, struct xbee_tbuf *buf, struct xbee_frameInfo *frameInfo, struct xbee_conAddress *address, struct xbee_pkt **pkt) {
	struct xbee_pkt *iPkt;
	xbee_err ret;
...
XBEE_DECLARE_CONTYPE(xbee1, modemStatus) = {
	.name = "Modem Status",
	.allowFrameId = 0,
	.useTimeout = 0,
	.addressRules = ADDR_NONE,
...
XBEE_DECLARE_CONTYPE_ARRAY(xbee1);
XBEE_DECLARE_MODE(xbee1) = {
	.name = "xbee1",
