...
xbee_err xbee_s6_at_rx_func(struct xbee *xbee, void *arg, unsigned char identifier, struct xbee_tbuf *buf, struct xbee_frameInfo *frameInfo, struct xbee_conAddress *address, struct xbee_pkt **pkt) {
...
xbee_err xbee_s6_at_rx_funcPost(struct xbee *xbee, struct xbee_con *con, struct xbee_pkt *pkt) {
...
xbee_err xbee_s6_at_tx_func(struct xbee *xbee, struct xbee_con *con, void *arg, unsigned char identifier, unsigned char frameId, struct xbee_conAddress *address, struct xbee_conSettings *settings, const unsigned char *buf, int len, struct xbee_sbuf **oBuf) {
	...
	if (len > 33) return XBEE_ELENGTH; /* 33 is max SSID + 2 */
	...
	memSize = 2;
	if (addr64) memSize += 8 + 1; /* 64bit and options */
	memSize += len;
	bufLen = memSize;
...
struct xbee_modeDataHandlerRx xbee_s6_localAt_rx  = {
	.identifier = 0x88,
	.func = xbee_s6_at_rx_func,
	.funcPost = xbee_s6_at_rx_funcPost,
};
struct xbee_modeDataHandlerTx xbee_s6_localAt_tx  = {
	.identifier = 0x08,
	.func = xbee_s6_at_tx_func,
};
struct xbee_modeConType xbee_s6_localAt = {
	.name = "Local AT",
	.allowFrameId = 1,
	.useTimeout = 1,
	.timeout = {
	...
	.rxHandler = &xbee_s6_localAt_rx,
	.txHandler = &xbee_s6_localAt_tx,
};

/* ######################################################################### */

struct xbee_modeDataHandlerRx xbee_s6_remoteAt_rx  = {
	.identifier = 0x87,
	.func = xbee_s6_at_rx_func,
	.funcPost = xbee_s6_at_rx_funcPost,
};
struct xbee_modeDataHandlerTx xbee_s6_remoteAt_tx  = {
	.identifier = 0x07,
	.func = xbee_s6_at_tx_func,
};
struct xbee_modeConType xbee_s6_remoteAt = {
	.name = "Remote AT",
	.allowFrameId = 1,
	.useTimeout = 1,
	.timeout = {
	...
	.rxHandler = &xbee_s6_remoteAt_rx,
	.txHandler = &xbee_s6_remoteAt_tx,
};
