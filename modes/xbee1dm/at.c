...
xbee_err xbee_s1dm_at_rx_func(struct xbee *xbee, void *arg, unsigned char identifier, struct xbee_tbuf *buf, struct xbee_frameInfo *frameInfo, struct xbee_conAddress *address, struct xbee_pkt **pkt) {
...
xbee_err xbee_s1dm_at_rx_funcPost(struct xbee *xbee, struct xbee_con *con, struct xbee_pkt *pkt) {
	if (!strncasecmp((char*)pkt->atCommand, "IS", 2)) {
		xbee_s1dm_io_parseInputs(xbee, pkt, pkt->data, pkt->dataLen);
	}
	
	return XBEE_ENONE;
}

xbee_err xbee_s1dm_at_tx_func(struct xbee *xbee, struct xbee_con *con, void *arg, unsigned char identifier, unsigned char frameId, struct xbee_conAddress *address, struct xbee_conSettings *settings, const unsigned char *buf, int len, struct xbee_sbuf **oBuf) {
...
struct xbee_modeDataHandlerRx xbee_s1dm_localAt_rx  = {
	.identifier = 0x88,
	.func = xbee_s1dm_at_rx_func,
	.funcPost = xbee_s1dm_at_rx_funcPost,
};
struct xbee_modeDataHandlerTx xbee_s1dm_localAt_tx  = {
	.identifier = 0x08,
	.func = xbee_s1dm_at_tx_func,
};
struct xbee_modeConType xbee_s1dm_localAt = {
	.name = "Local AT",
	.allowFrameId = 1,
	.useTimeout = 1,
	.timeout = {
	...
	.rxHandler = &xbee_s1dm_localAt_rx,
	.txHandler = &xbee_s1dm_localAt_tx,
};

/* ######################################################################### */

struct xbee_modeDataHandlerRx xbee_s1dm_remoteAt_rx  = {
	.identifier = 0x97,
	.func = xbee_s1dm_at_rx_func,
	.funcPost = xbee_s1dm_at_rx_funcPost,
};
struct xbee_modeDataHandlerTx xbee_s1dm_remoteAt_tx  = {
	.identifier = 0x17,
	.func = xbee_s1dm_at_tx_func,
};
struct xbee_modeConType xbee_s1dm_remoteAt = {
	.name = "Remote AT",
	.allowFrameId = 1,
	.useTimeout = 1,
	.timeout = {
	...
	.save_addr16 = 1,
	.rxHandler = &xbee_s1dm_remoteAt_rx,
	.txHandler = &xbee_s1dm_remoteAt_tx,
};
