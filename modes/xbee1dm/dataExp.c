...
xbee_err xbee_s1dm_dataExp_rx_func(struct xbee *xbee, void *arg, unsigned char identifier, struct xbee_tbuf *buf, struct xbee_frameInfo *frameInfo, struct xbee_conAddress *address, struct xbee_pkt **pkt) {
	...
	address->cluster_id = (buf->data[13] << 8) & 0xFF00;
	address->cluster_id |= buf->data[14];

	address->profile_enabled = 1;
	address->profile_id = (buf->data[15] << 8) & 0xFF00;
	address->profile_id |= buf->data[16];
...
xbee_err xbee_s1dm_dataExp_tx_func(struct xbee *xbee, struct xbee_con *con, void *arg, unsigned char identifier, unsigned char frameId, struct xbee_conAddress *address, struct xbee_conSettings *settings, const unsigned char *buf, int len, struct xbee_sbuf **oBuf) {
	...
	if (address->cluster_enabled) {
		...
		iBuf->data[pos] = 0x00; /* default cluster ID */    pos++;
		iBuf->data[pos] = 0x11;                             pos++;
	}
	if (address->profile_enabled) {
		iBuf->data[pos] = (address->profile_id >> 8) & 0xFF;pos++;
		iBuf->data[pos] =  address->profile_id       & 0xFF;pos++;
	} else {
		iBuf->data[pos] = 0xC1; /* default profile ID */    pos++;
		iBuf->data[pos] = 0x05;                             pos++;
	}
...
struct xbee_modeDataHandlerRx xbee_s1dm_dataExp_rx  = {
	.identifier = 0x91,
	.func = xbee_s1dm_dataExp_rx_func,
};
struct xbee_modeDataHandlerTx xbee_s1dm_dataExp_tx  = {
	.identifier = 0x11,
	.func = xbee_s1dm_dataExp_tx_func,
};
struct xbee_modeConType xbee_s1dm_dataExp = {
	.name = "Data (explicit)",
	.allowFrameId = 1,
	.useTimeout = 0,
	...
	.save_addr16 = 1,
	.rxHandler = &xbee_s1dm_dataExp_rx,
	.txHandler = &xbee_s1dm_dataExp_tx,
};
