...
xbee_err xbee_s3_at_rx_func(struct xbee *xbee, void *arg, unsigned char identifier, struct xbee_tbuf *buf, struct xbee_frameInfo *frameInfo, struct xbee_conAddress *address, struct xbee_pkt **pkt) {
...
xbee_err xbee_s3_at_rx_funcPost(struct xbee *xbee, struct xbee_con *con, struct xbee_pkt *pkt) {
	if (!strncasecmp((char*)pkt->atCommand, "IS", 2)) {
		xbee_s3_io_parseInputs(xbee, pkt, pkt->data, pkt->dataLen);
	}
	
	return XBEE_ENONE;
}

xbee_err xbee_s3_at_tx_func(struct xbee *xbee, struct xbee_con *con, void *arg, unsigned char identifier, unsigned char frameId, struct xbee_conAddress *address, struct xbee_conSettings *settings, const unsigned char *buf, int len, struct xbee_sbuf **oBuf) {
	...
	switch (identifier) {
		...
		case 0x17: /* Remote AT */
			...
			memSize += 8 + 2 + 1; /* 64bit, 16bit/reserved and options */
			break;
		default: return XBEE_EINVAL;
	}
	...
	if (identifier == 0x17) {
		...
		/* hole from 16-bit address is 'reserved' */
